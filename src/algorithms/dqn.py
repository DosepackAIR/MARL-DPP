...
class RL:
    def __init__(self, input_dim, output_dim, cars, sess, env_config, learning_rate=0.0001, decay_rate=0.000001, batch_size=32,
    ...
    def replay(self, replay_given_state=None):
        ...
        if replay_given_state is not None:
            ...
            # p = p / sum(p)
            # size = self.batch_size if memory < 64 else 64
        ...
        #                                          axis=1)
        true = rewards + self.discount * np.amax(self.target_model.predict(next_states), axis=1)
        ...
        self.model.fit(x_train, y_train, batch_size=self.batch_size, callbacks=None, verbose=0,
                       epochs=self.epochs, validation_data=None)
    ...
    def train_given_state(self, episodes, inputfile_number, outputfile_number, input_car_states=None, no_of_cars=1,
        ...
        if self.load_model_flag[0]:
            ...
            self.model = load_model(dir[:len(dir) - 10] + 'models/' + self.load_model_flag[1])
    ...
    def train_given_state_random_car_order(self, episodes, inputfile_number, outputfile_number, input_car_states=None,
                                           no_of_cars=1, destination_index_array=[0, 0, 0, 0],
                                           destination_choose_arrays=[0, 0, 0]):
        dir = os.path.dirname(os.path.realpath(__file__))
        # model_name = 'model-0.' + str(datetime.datetime.now()) + '.h5'
        # self.model.save(dir[:len(dir) - 10] + 'models/' + model_name)
        # model_name = 'model-0.' + str(datetime.datetime.now()) + '.h5'
        ...
        for episode in range(1, episodes + 1):
            ...
            while not self.training_done:
                ...
                self.replay()
                next_state_array = env.state_to_array(next_state)
                # self.update_car_logger.info('Entering Update Car')
                env.update_car(num, deepcopy(next_state_array[0][0]), deepcopy(next_state_array[0][1]), 1)
                if self.epsilon <= 0:
                    pass
            ...
            # print("Episode: {}, learning-rate: {}".format(episode, float(K.get_value(self.model.optimizer.lr))))
